...
function scssTask3() {
   return src("public/scss/chat/style.scss", { sourcemaps: true }) // selecting the style/main scss file || { sourcemaps: true } will generate a .css.map file (show which scss file  the styles are coming from)
      .pipe(sass()) // running sass function to compile scss to css
      .pipe(postcss([autoprefixer(), cssnano()])) // using autoprefixer function will add the browser prefixes to support older browsers || and cssnano function to minify css file
      .pipe(dest("dist/chat", { sourcemaps: "." })); // seting the destination of the compiled css and the map file to a folder called dist
}
...
function jsTask3() {
   return src("public/js/chat/script.js", { sourcemaps: true }) // selecting the js file || { sourcemaps: true } will generate a .js.map file (show which js file  the scripts are coming from)
      .pipe(babel({ presets: ["@babel/preset-env"] })) // running babel function to compile JavaScript to older versions so old browsers can run it with no problems
      .pipe(terser()) // using terser function to minify JavaScript file
      .pipe(dest("dist/chat", { sourcemaps: "." })); // seting the destination of the compiled JavaScript and the map file to a folder called dist
}
